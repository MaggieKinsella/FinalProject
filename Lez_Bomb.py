...
class ogButton (PhaseThread):
  colors = ["R", "G", "B" ] #the button's possible colors
  def __init__(self, lcd, state, rgd, name="Button"):
    super().__init__(name)
    self.lcd = lcd
    self._value = False
    self._state = state
  ...
  def run(self):
    ...
    def run(self):
        ...
        rgb_counter = 0
        while True:
            self._rgb[0].value = False if ogButton.colors[rgb_index] == "R" else True  # Red
            self._rgb[1].value = False if ogButton.colors[rgb_index] == "G" else True  # Green
            self._rgb[2].value = False if ogButton.colors[rgb_index] == "B" else True  # Blue
